...
(function() {
    ...
    async function processOrthodromeIntersection(zeroHorizonAzimuth, locationData, viewshedPolyline, scenarioName, layerGroupName) {
        ...
        if (!isNaN(zeroHorizonAzimuth) && locationData && typeof LatLon !== 'undefined' && typeof L.geodesic !== 'undefined') {
            ...
            console.log(`Raw Geodesic Orthodrome Attempt LatLngs for ${scenarioName}:`, rawGeodesicLatLngs);
            ...
            if (viewshedPolyline && typeof turf !== 'undefined') {
                ...
                if (intersectionPoint) {
                    ...
                    if (intersectionMarker) {
                        ...
                        console.log(`Intersection marker layer group added to map for ${scenarioName}.`);
                    }
                } else {
        ...
        for (const point of horizonData) {
            ...
            if (point && typeof point.horizonLat === 'number' && !isNaN(point.horizonLat) &&
                ...
                if (!seenLocations.has(key)) {
                    ...
                    // Log if a duplicate is found (optional, for debugging)
                    console.log(`Deduplicate: Skipping duplicate Lat/Lon: ${key} (Azimuth: ${point.azimuth})`);
                }
            } else {
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        form.addEventListener('submit', async (e) => {
            ...
            // --- Log Declination Values from Script A's Global Variables ---
            console.log("Summer Solstice Declination (from window.declinationSummerSolstice):", window.declinationSummerSolstice ? window.declinationSummerSolstice.toFixed(4) : "undefined/null");
            console.log("Winter Solstice Declination (from window.declinationWinterSolstice):", window.declinationWinterSolstice ? window.declinationWinterSolstice.toFixed(4) : "undefined/null");
            // --- END Log Declination Values ---
            ...
            console.log(`SSR Declination used: ${declinationDegSSR}°`);
            console.log(`WSR Declination used: ${declinationDegWSR}°`);
            ...
            if (ssrIntersectionPoint && !isNaN(ssrIntersectionPoint.azimuth)) {
                ...
                console.log(`Using Turf intersection azimuth (${baseAzimuthForPolygonSSR.toFixed(3)}°) as base for SSR polygon.`);
            } else {
                console.log(`Using original 0-horizon azimuth (${ssrZeroHorizonAzimuth.toFixed(3)}°) as base for SSR polygon (no valid SSR intersection).`);
            }
            ...
            if (wsrIntersectionPoint && !isNaN(wsrIntersectionPoint.azimuth)) {
                ...
                console.log(`Using Turf intersection azimuth (${baseAzimuthForPolygonWSR.toFixed(3)}°) as base for WSR polygon.`);
            } else {
                console.log(`Using original 0-horizon azimuth (${wsrZeroHorizonAzimuth.toFixed(3)}°) as base for WSR polygon (no valid WSR intersection).`);
            }
            ...
            var ssrResult = await createAndAddPolygonLayer(
                ...
                isSunriseLikeSSR, declinationDegSSR // Pass new parameters
            );
            ...
            var wsrResult = await createAndAddPolygonLayer(
                ...
                isSunriseLikeWSR, declinationDegWSR // Pass new parameters
            );
