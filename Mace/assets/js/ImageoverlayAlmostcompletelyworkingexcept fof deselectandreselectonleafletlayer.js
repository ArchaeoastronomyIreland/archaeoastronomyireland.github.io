        ...
        $(document).ready(function() {
            ...
            map.on('overlayadd', function(e) {
                ...
                if (originalFileName && overlaidImagesData[originalFileName]) {
                    ...
                    if (storedData.originalMimeType === 'image/svg+xml') { // Use originalMimeType for this check
                    ...
                    } else if (storedData.originalMimeType.startsWith('image/') && Array.isArray(storedData.lastKnownState) && storedData.lastKnownState.length === 4 && storedData.lastKnownState[0].lat !== undefined) {
                    ...
                    } else if (storedData.originalMimeType.startsWith('image/')) {
            ...
            processZipFiles().then(() => {
                ...
                const processNextImage = () => {
                    if (currentImageIndex < totalImages) {
                        ...
                        // Store the original MIME type for later export logic
                        ...
                        if (imageFile.type === 'image/svg+xml') {
                            ...
                            // Pre-process JPG/GIF to PNG immediately on import
                            readerMethod = 'readAsDataURL'; // Read as data URL first
                            fileMimeType = 'image/png'; // It will become a PNG
                        }
                        ...
                        reader.onload = (e) => {
                            ...
                            if (originalFileMimeType === 'image/svg+xml') {
                                ...
                                // Convert loaded JPG/GIF data URL to PNG data URL via a temporary canvas
                                ...
                                tempImg.onerror = () => {
                                    console.error(`Failed to load temp image for conversion: ${originalFileName}.`);
                                    filesFailedCount++;
                                    filesProcessedTotal++;
                                    statusMessage.textContent = `Error: Failed to convert image ${originalFileName}.`;
                                    statusMessage.classList.remove('text-muted', 'text-success');
                                    statusMessage.classList.add('text-danger');
                                    updateProgressDisplay();
                                    currentImageIndex++;
                                ...
                                tempImg.src = e.target.result; // The original data URL of the JPG/GIF
                            } else {
                                // For PNGs, use the base64 data URL directly
        ...
        async function handleUrlUpload() {
            ...
            try {
                ...
                reader.onload = (e) => {
                    ...
                    let originalFileMimeType = blob.type; // Store original MIME type
                    ...
                    if (originalFileMimeType === 'image/svg+xml') {
                        ...
                        tempImg.onload = () => {
                            ...
                            ctx.drawImage(tempImg, 0, 0);
                            const pngDataUrl = canvas.toDataURL('image/png');
                            addImageOverlay(pngDataUrl, fileName, null, 'image/png', originalFileMimeType); // Pass 'image/png' as current mimeType
                            statusMessage.textContent = `Image from URL "${fileName}" uploaded successfully.`;
                            statusMessage.classList.remove('text-muted', 'text-danger');
                            statusMessage.classList.add('text-success');
                            imageUrlInput.value = '';
