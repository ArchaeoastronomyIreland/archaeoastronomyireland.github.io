...
   $('#minor4long').html(minor4long);
    ...
    // --- NEW CODE START ---
    // Expose declination variables globally for Script C
    window.declinationSummerSolstice = obliquity * 180 / Math.PI;
    window.declinationWinterSolstice = -obliquity * 180 / Math.PI;
    window.declinationEquinox = hoek2 * 180 / Math.PI; // hoek2 is 0 if equinox is active
    window.declinationCrossQuarterNorth = hoek3 * 180 / Math.PI; // hoek3 is 0.69 * obliquity if active
    window.declinationCrossQuarterSouth = -hoek3 * 180 / Math.PI;
    window.declinationMajorLunarNorth = hoek4 * 180 / Math.PI; // hoek4 includes lunar perturbation
    window.declinationMajorLunarSouth = -hoek4 * 180 / Math.PI;
    window.declinationMinorLunarNorth = hoek5 * 180 / Math.PI; // hoek5 includes lunar perturbation
    window.declinationMinorLunarSouth = -hoek5 * 180 / Math.PI;

    // Output declination values to console for verification
    console.log("--- Script A Declination Values for Script C ---");
    console.log("Summer Solstice Declination:", window.declinationSummerSolstice.toFixed(4));
    console.log("Winter Solstice Declination:", window.declinationWinterSolstice.toFixed(4));
    console.log("Equinox Declination:", window.declinationEquinox.toFixed(4));
    console.log("North Cross-Quarter Declination:", window.declinationCrossQuarterNorth.toFixed(4));
    console.log("South Cross-Quarter Declination:", window.declinationCrossQuarterSouth.toFixed(4));
    console.log("North Major Lunar Declination:", window.declinationMajorLunarNorth.toFixed(4));
    console.log("South Major Lunar Declination:", window.declinationMajorLunarSouth.toFixed(4));
    console.log("North Minor Lunar Declination:", window.declinationMinorLunarNorth.toFixed(4));
    console.log("South Minor Lunar Declination:", window.declinationMinorLunarSouth.toFixed(4));
    console.log("--- End Script A Declination Values ---");
    // --- NEW CODE END ---
