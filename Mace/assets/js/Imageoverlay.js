        ...
        if (typeof L.ExportAction !== 'undefined' && typeof html2canvas !== 'undefined') {
            L.ExportAction.prototype.addHooks = function () {
                ...
                // If the original was a JPG/GIF, it's now stored as PNG, so the filename in JSON should reflect that.
                let imageFileNameInZip = overlayFileName;
                if (storedImageData.originalMimeType !== 'image/svg+xml') { // Check originalMimeType to know if it was a raster
                ...
                if (storedImageData.originalMimeType === 'image/svg+xml') { // Use originalMimeType for this check
                    ...
                    // These are now guaranteed to be PNG data URLs internally after import preprocessing
        ...
        $(document).ready(function() {
            ...
            map.on('overlayremove', function(e) {
                ...
                if (originalFileName && overlaidImagesData[originalFileName]) {
                    ...
                    // Save the current state of the layer when it's removed
                    if (e.layer.getCorners && typeof e.layer.getCorners === 'function') {
                        data.lastKnownState = e.layer.getCorners();
                    } else if (e.layer.getBounds && typeof e.layer.getBounds === 'function') {
            ...
            map.on('overlayadd', function(e) {
                ...
                if (originalFileName && overlaidImagesData[originalFileName]) {
                        ...
                        if (layerswitcher._layers) {
                            ...
                            if (storedData.currentLayerRef) {
                                for (let i in layerswitcher._layers) {
                                    ...
                                    // If a layer with the same originalFileName exists but is a different instance, remove it.
                                    // This handles cases where a new instance might have been created but not properly cleaned up.
                                    if (layerswitcher._layers[i].layer._originalFileName === originalFileName && layerswitcher._layers[i].layer !== e.layer) {
                                        layerswitcher.removeLayer(layerswitcher._layers[i].layer);
                                    }
                                }
                            }
                        }
                    ...
                    // Use the stored 'type' to determine how to re-create the layer
                    if (storedData.type === 'distortable') {
                        // This applies to SVGs and raster images that are currently interactive
                        // (either newly added, or re-added after being temporarily hidden).
                                ...
                                return new Promise((resolve, reject) => {
                                    if (this.getElement()) {
                                        html2canvas(this.getElement(), {
                                            backgroundColor: null, useCORS: true, allowTaint: true, scale: 2,
                        ...
                        console.log(`Re-added '${originalFileName}' as interactive DistortableImageOverlay.`);
                    } else if (storedData.type === 'staticRaster') {
                        // This applies to raster images loaded from zip that were already baked-in distorted or simple rectangular.
                        // They should always be static L.imageOverlay instances.
                        let boundsToUse = storedData.lastKnownState;
                        // If lastKnownState is corners (from a previously distorted raster that was exported), derive bounds.
                        if (Array.isArray(storedData.lastKnownState) && storedData.lastKnownState.length === 4 && storedData.lastKnownState[0].lat !== undefined) {
                            ...
                            boundsToUse = L.latLngBounds([minLat, minLng], [maxLat, maxLng]);
                        }
                        newLayer = L.imageOverlay(storedData.dataUrl, boundsToUse, {
                            opacity: storedData.lastKnownOpacity,
                            interactive: false // Always static
                        });
                        console.log(`Re-added '${originalFileName}' as static L.imageOverlay.`);
                    } else {
                        // Fallback: This case should ideally not be hit if 'type' is always set correctly.
                        console.warn(`Attempting fallback to DistortableImageOverlay for unknown stored type: ${originalFileName}`);
                        layerOptions.corners = storedData.lastKnownState;
                        newLayer = new L.DistortableImageOverlay(storedData.dataUrl, layerOptions);
                        ...
                        // Set type to distortable as a fallback
                        storedData.type = 'distortable';
                    }
        ...
        // Helper function to convert image data URL to PNG data URL
        function convertImageToPngDataUrl(dataUrl, originalFileName) {
            return new Promise((resolve, reject) => {
                ...
                tempImg.onerror = (e) => {
                    console.error(`Failed to load image for PNG conversion: ${originalFileName}`, e);
                    reject(new Error(`Failed to convert ${originalFileName} to PNG.`));
                };
                tempImg.src = dataUrl;
            });
        }
        ...
        function handleFiles(files) {
                                ...
                                if (zipImage && zipJson && imageMimeType) {
                                    ...
                                    Promise.all([
                                        ...
                                        if (imageMimeType === 'image/svg+xml') {
                                            // Robust UTF-8 to Base64 conversion for SVG
                                            const binaryString = new TextEncoder().encode(imageDataContent).reduce((acc, byte) => acc + String.fromCharCode(byte), '');
                                            dataUrl = `data:${imageMimeType};base64,${btoa(binaryString)}`;
                                        } else {
                                        ...
                                        try {
                                            ...
                                            if (georefData.originalFileName === imageFileName) {
                                                ...
                                                let storedDataType; // New variable to store the 'type' for overlaidImagesData
                                                ...
                                                if (imageMimeType === 'image/svg+xml') {
                                                    ...
                                                    storedDataType = 'distortable'; // Set type for SVG
                                                    // Ensure export method is attached for new DistortableImageOverlay instances
                                                    ...
                                                    storedDataType = 'staticRaster'; // Set type for static distorted raster
                                                    ...
                                                    storedDataType = 'staticRaster'; // Set type for simple rectangular raster
                                                }
                                                ...
                                                overlaidImagesData[imageFileName] = {
                                                    ...
                                                    originalMimeType: imageMimeType, // Store original MIME type for export logic
                                                    type: storedDataType, // Store the explicit type
            ...
            processZipFiles().then(() => {
                    ...
                    jsonFiles.forEach(jsonFile => {
                        ...
                        reader.onload = (e) => {
                            try {
                                ...
                                filesFailedCount++; // Mark as failed
                ...
                const totalFiles = imageFiles.length; // Use totalFiles for the loop condition

                const processNextImage = () => {
                    if (currentImageIndex < totalFiles) {
                        const imageFile = imageFiles[currentImageIndex];
                        const reader = new FileReader();

                        let readerMethod = 'readAsDataURL';
                        let fileMimeType = imageFile.type;
                        let originalFileMimeType = imageFile.type;

                        if (imageFile.type === 'image/svg+xml' || imageFile.name.toLowerCase().endsWith('.svg')) {
                            readerMethod = 'readAsText';
                            fileMimeType = 'image/svg+xml';
                            originalFileMimeType = 'image/svg+xml';
                        } else if (imageFile.type === 'image/jpeg' || imageFile.type === 'image/gif') {
                            readerMethod = 'readAsDataURL';
                            fileMimeType = 'image/png'; // Will be converted to PNG
                        }

                        reader.onload = (e) => {
                            const originalFileName = imageFile.name;
                            const georef = pendingGeorefData[originalFileName];

                            if (originalFileMimeType === 'image/svg+xml') {
                                // Robust UTF-8 to Base64 conversion for SVG
                                const binaryString = new TextEncoder().encode(e.target.result).reduce((acc, byte) => acc + String.fromCharCode(byte), '');
                                const dataUrl = `data:image/svg+xml;base64,${btoa(binaryString)}`;
                                addImageOverlay(dataUrl, originalFileName, georef, fileMimeType, originalFileMimeType);
                                
                                if (georef) {
                                    delete pendingGeorefData[originalFileName];
                                ...
                                currentImageIndex++;
                                processNextImage();
                            } else if (originalFileMimeType === 'image/jpeg' || originalFileMimeType === 'image/gif') {
                                convertImageToPngDataUrl(e.target.result, originalFileName)
                                    .then(pngDataUrl => {
                                        addImageOverlay(pngDataUrl, originalFileName, georef, fileMimeType, originalFileMimeType);
                                        if (georef) {
                                            delete pendingGeorefData[originalFileName];
                                            statusMessage.textContent = `Placed image '${originalFileName}' using georef data.`;
                                        } else {
                            ...
                            } else { // For PNGs (or other non-SVG, non-JPG/GIF rasters)
                                addImageOverlay(e.target.result, originalFileName, georef, fileMimeType, originalFileMimeType);
        ...
        async function handleUrlUpload() {
            ...
            try {
                ...
                reader.onload = (e) => {
                    ...
                    let originalFileMimeType = blob.type;

                    if (!mimeType) {
                        const ext = fileName.split('.').pop().toLowerCase();
                        if (ext === 'svg') { mimeType = 'image/svg+xml'; originalFileMimeType = 'image/svg+xml'; }
                        else if (ext === 'png') { mimeType = 'image/png'; originalFileMimeType = 'image/png'; }
                        else if (ext === 'jpg' || ext === 'jpeg') { mimeType = 'image/jpeg'; originalFileMimeType = 'image/jpeg'; }
                        else if (ext === 'gif') { mimeType = 'image/gif'; originalFileMimeType = 'image/gif'; }
                        else { mimeType = 'application/octet-stream'; originalFileMimeType = 'application/octet-stream'; }
                    }

                    if (originalFileMimeType === 'image/svg+xml') {
                        const binaryString = new TextEncoder().encode(e.target.result).reduce((acc, byte) => acc + String.fromCharCode(byte), '');
                        const dataUrlToPass = `data:image/svg+xml;base64,${btoa(binaryString)}`;
                        addImageOverlay(dataUrlToPass, fileName, null, mimeType, originalFileMimeType);
                        statusMessage.textContent = `Image from URL "${fileName}" uploaded successfully.`;
                        statusMessage.classList.remove('text-muted', 'text-danger');
                        statusMessage.classList.add('text-success');
                        imageUrlInput.value = '';
                    } else if (originalFileMimeType === 'image/jpeg' || originalFileMimeType === 'image/gif') {
                        convertImageToPngDataUrl(e.target.result, fileName)
                            .then(pngDataUrl => {
                                addImageOverlay(pngDataUrl, fileName, null, 'image/png', originalFileMimeType);
                                statusMessage.textContent = `Image from URL "${fileName}" uploaded successfully.`;
                                statusMessage.classList.remove('text-muted', 'text-danger');
                                statusMessage.classList.add('text-success');
                                imageUrlInput.value = '';
                        ...
                        const dataUrlToPass = e.target.result;
                        addImageOverlay(dataUrlToPass, fileName, null, mimeType, originalFileMimeType);
                        statusMessage.textContent = `Image from URL "${fileName}" uploaded successfully.`;
                        statusMessage.classList.remove('text-muted', 'text-danger');
                        statusMessage.classList.add('text-success');
         ...
         * @param {string} originalMimeType - The original MIME type of the file before any conversion (e.g., 'image/jpeg').
         */
        function addImageOverlay(dataUrl, originalFileName, georefData, mimeType, originalMimeType) {
            const img = new Image();
            img.onload = () => {
                ...
                overlaidImagesData[originalFileName] = {
                    dataUrl: dataUrl,
                    mimeType: mimeType, // This is the *current* MIME type (e.g., image/png if converted)
                    originalMimeType: originalMimeType, // This is the *original* MIME type (e.g., image/jpeg)
                    type: 'distortable', // All newly added images are interactive distortable overlays
            ...
            img.onerror = () => {
                ...
                const initialOverlayOptions = {
                    ...
                    actions: [
                        L.BorderAction,
                        L.DeleteAction,
                        L.DragAction, // Keep DragAction for basic positioning
                        L.FreeRotateAction,
                        L.LockAction,
                        L.OpacityAction,
                        L.RotateAction,
                ...
                overlaidImagesData[originalFileName] = {
                    ...
                    mimeType: mimeType, // This is the *current* MIME type (e.g., image/png if converted)
                    originalMimeType: originalMimeType, // This is the *original* MIME type (e.g., image/jpeg)
                    type: 'distortable', // Even broken images should attempt to be distortable if possible
                    lastKnownState: initialOverlay.getCorners(),
                    lastKnownOpacity: initialOverlay.options.opacity,
                    elementId: elementId,
                    currentLayerRef: initialOverlay,
