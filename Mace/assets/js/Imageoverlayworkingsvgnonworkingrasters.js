        ...
        $(document).ready(function() {
            ...
            map.on('overlayadd', function(e) {
                ...
                if (originalFileName && overlaidImagesData[originalFileName]) {
                    ...
                    let newLayer;
                    // Recreate layer based on stored mimeType and lastKnownState
                    const layerOptions = {
                    ...
                    if (storedData.mimeType === 'image/svg+xml' || (Array.isArray(storedData.lastKnownState) && storedData.lastKnownState.length === 4 && storedData.lastKnownState[0].lat !== undefined)) {
                        // If it's an SVG or if lastKnownState explicitly indicates 4 corners (meaning it was distorted),
                        // use DistortableImageOverlay with the stored corners.
                    ...
                    } else if (storedData.mimeType.startsWith('image/') && !Array.isArray(storedData.lastKnownState)) {
                        // For raster images that were simple L.imageOverlay (not distorted)
                        layerOptions.bounds = storedData.lastKnownState; // storedData.lastKnownState should be bounds
                        ...
                        // Fallback: If type is unknown or state is not clearly corners/bounds, try to recreate as distortable
                                    ...
                                    Promise.all([
                                        ...
                                        try {
                                            ...
                                            if (georefData.originalFileName === imageFileName) {
                                                ...
                                                if (imageMimeType === 'image/svg+xml' || (georefData.corners && georefData.corners.length === 4)) {
                                                    // For SVGs from zip, or if georefData has explicit corners (distorted), treat as distortable
                                                    ...
                                                    layerControlType = imageMimeType === 'image/svg+xml' ? 'Distortable SVG' : 'Distortable Raster';
                                                    // Ensure export method for this new instance
                                                    ...
                                                    // For raster images from zip that are not distorted, use L.imageOverlay
