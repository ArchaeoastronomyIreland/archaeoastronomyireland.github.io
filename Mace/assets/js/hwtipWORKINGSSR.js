...
(function() {
    ...
    window.findActualAzimuthForTargetApparentAltitude = async function( // Exposed globally
        ...
        for (let ha = -180; ha <= 180; ha += HA_SEARCH_RESOLUTION) {
            ...
            console.log(`  Coarse Search HA: ${ha.toFixed(3)}°, Sun Az: ${sunPos.azimuth.toFixed(3)}°, Sun Apparent Center Alt: ${sunApparentCenterAltitude.toFixed(3)}°, Terrain Az: ${sunPos.azimuth.toFixed(3)}°, Terrain Alt: ${terrainAltitude.toFixed(3)}°, Limb Adj: ${limbAdjustmentValue.toFixed(3)}°, Calculated Diff: ${currentDifference.toFixed(3)}°, Above Terrain: ${isSunAboveTerrain}`);
        ...
        if (!bestBracket) {
            ...
            const testSunApparentCenterAltitude = testSunPos.altitude + SOLAR_REFRACTION;
        ...
        for (let i = 0; i < MAX_BISECTION_ITERATIONS; i++) {
            ...
            console.log(`  Bisection Step ${i}: HA: ${midHA.toFixed(3)}°, Sun Az: ${midSunPos.azimuth.toFixed(3)}°, Sun Apparent Center Alt: ${midSunApparentCenterAltitude.toFixed(3)}°, Terrain Az: ${midSunPos.azimuth.toFixed(3)}°, Terrain Alt: ${midTerrainAlt.toFixed(3)}°, Limb Adj: ${limbAdjustmentValue.toFixed(3)}°, Calculated Diff: ${currentDifference.toFixed(3)}°`);
     ...
     * @param {string} layerGroupName - Name for the layer group to be displayed in Leaflet's layers control.
    ...
    async function processOrthodromeIntersection(zeroHorizonAzimuth, locationData, viewshedPolyline, scenarioName, layerGroupName) {
        ...
        if (!isNaN(zeroHorizonAzimuth) && locationData && typeof LatLon !== 'undefined' && typeof L.geodesic !== 'undefined') {
            ...
            if (viewshedPolyline && typeof turf !== 'undefined') {
                ...
                let orthodromeLayerGroup = L.layerGroup();
                orthodromeLayerGroup.addLayer(tempOrthodromeLine); // Add the orthodrome line to the group
                ...
                if (intersectionPoint) {
                    ...
                    if (intersectionMarker) {
                        orthodromeLayerGroup.addLayer(intersectionMarker);
                        console.log(`Intersection marker added to layer group for ${scenarioName}.`);
                    }
                } else {
                    console.warn(`No intersection found between ${scenarioName} Orthodrome and Viewshed Horizon, or data invalid.`);
                }

                orthodromeLayerGroup.layerNameForControl = layerGroupName;
                map.addLayer(orthodromeLayerGroup);
                layersControl.addOverlay(orthodromeLayerGroup, orthodromeLayerGroup.layerNameForControl);
                window.scriptCOverlayGroups.push(orthodromeLayerGroup);
                console.log(`Orthodrome layer group for ${scenarioName} added to map.`);
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        form.addEventListener('submit', async (e) => {
            ...
            // The initialAzimuthGuess for findActualAzimuthForTargetApparentAltitude is now purely conceptual.
            // The function's internal coarse search will find the correct starting HA from -180.
            ...
            if (actualPointUL_SSR) {
                let ulMarker = drawIndividualPointMarker(actualPointUL_SSR, "SSR Upper Limb", POLYGON_COLORS[0], 6, 1); // Using POLYGON_COLORS[0] (orange)
                if (ulMarker) {
                    let ulLayerGroup = L.layerGroup();
                    ulLayerGroup.addLayer(ulMarker);
                    ulLayerGroup.layerNameForControl = "SSR Upper Limb";
                    window.map.addLayer(ulLayerGroup);
                    window.layersControl.addOverlay(ulLayerGroup, ulLayerGroup.layerNameForControl);
                    window.scriptCOverlayGroups.push(ulLayerGroup);
                    console.log("SSR Upper Limb marker added to map.");
                } else {
            ...
            if (!anyCalculationFailed) {
                window.displayMessage('overallStatus', 'Summer Solstice Rise (Upper Limb) calculation complete.', 'success');
            } else {
                window.displayMessage('overallStatus', 'Summer Solstice Rise (Upper Limb) calculation finished with issues. Check console for more info.', 'warn');
            }
            if (loadingSpinner) loadingSpinner.classList.add('hidden');
        });
    });
