...
(function() {
    ...
    function drawViewshedHorizonLine(horizonData) {
        ...
        if (polygonPoints.length >= 2) {
            ...
            return L.polygon(polygonPoints, {
                color: '#808080', // Grey line color
                weight: 2,
                opacity: 0.7,
                fillColor: '#808080', // Grey fill color
    ...
    window.findActualAzimuthForTargetApparentAltitude = async function( // Exposed globally
        ...
        // originalConsole.log(`Using REFRACTION: ${REFRACTION.toFixed(4)}°, SEMIDIAMETER: ${SEMIDIAMETER.toFixed(4)}° (Is Lunar: ${isLunarEvent}, Is CrossQuarter: ${isCrossQuarterEvent})`);
        ...
        if (finalCelestialPos && !isNaN(finalCelestialPos.azimuth)) {
            ...
            if (finalPointLatLon && !isNaN(finalPointLatLon.lat) && !isNaN(finalPointLatLon.lon)) {
                ...
                // originalConsole.log(`Calculated ${scenarioName} Result: Azimuth: ${finalCelestialPos.azimuth.toFixed(3)}°, Lat: ${finalPointLatLon.lat.toFixed(6)}, Lon: ${finalPointLatLon.lon.toFixed(6)}, HA: ${finalHA.toFixed(3)}°`); // Removed as requested
                return {
        ...
        if (!isNaN(zeroHorizonAzimuth) && locationData && typeof LatLon !== 'undefined' && typeof L.geodesic !== 'undefined') {
            ...
            if (viewshedPolygon && typeof turf !== 'undefined') {
                ...
                if (intersectionPoint) {
                    // Orthodrome intersection markers: 50% current size (radius 2.5), Line Black, Fill White
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        form.addEventListener('submit', async (e) => {
            ...
            // Observer Location Marker: Size to remain (radius 5), Line Black, Fill White
            ...
            const ssrIntersectionPoint = await processOrthodromeIntersection(ssrZeroHorizonAzimuth, locationData, viewshedPolygon, "SSR", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_SSR) {
                ...
                let ulMarker = drawIndividualPointMarker(actualPointUL_SSR, "SSR Upper Limb", POLYGON_COLORS[0], POLYGON_COLORS[0], 2.5, 1);
                if (ulMarker) {
            ...
            if (actualPointCenter_SSR) {
                // Solstice markers: 50% current size (radius 2.5), Fill Orange
                let centerMarker = drawIndividualPointMarker(actualPointCenter_SSR, "SSR Center", POLYGON_COLORS[0], POLYGON_COLORS[0], 2.5, 1);
                if (centerMarker) {
            ...
            if (actualPointLL_SSR) {
                // Solstice markers: 50% current size (radius 2.5), Fill Orange
                let llMarker = drawIndividualPointMarker(actualPointLL_SSR, "SSR Lower Limb", POLYGON_COLORS[0], POLYGON_COLORS[0], 2.5, 1);
                if (llMarker) {
            ...
            const wsrIntersectionPoint = await processOrthodromeIntersection(wsrZeroHorizonAzimuth, locationData, viewshedPolygon, "WSR", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_WSR) {
                // Solstice markers: 50% current size (radius 2.5), Fill Orange
                let ulMarkerWSR = drawIndividualPointMarker(actualPointUL_WSR, "WSR Upper Limb", POLYGON_COLORS[0], POLYGON_COLORS[0], 2.5, 1);
                if (ulMarkerWSR) {
            ...
            if (actualPointCenter_WSR) {
                // Solstice markers: 50% current size (radius 2.5), Fill Orange
                let centerMarkerWSR = drawIndividualPointMarker(actualPointCenter_WSR, "WSR Center", POLYGON_COLORS[0], POLYGON_COLORS[0], 2.5, 1);
                if (centerMarkerWSR) {
            ...
            if (actualPointLL_WSR) {
                // Solstice markers: 50% current size (radius 2.5), Fill Orange
                let llMarkerWSR = drawIndividualPointMarker(actualPointLL_WSR, "WSR Lower Limb", POLYGON_COLORS[0], POLYGON_COLORS[0], 2.5, 1);
                if (llMarkerWSR) {
            ...
            const sssIntersectionPoint = await processOrthodromeIntersection(sssZeroHorizonAzimuth, locationData, viewshedPolygon, "SSS", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_SSS) {
                // Solstice markers: 50% current size (radius 2.5), Fill Orange
                let ulMarkerSSS = drawIndividualPointMarker(actualPointUL_SSS, "SSS Upper Limb", POLYGON_COLORS[0], POLYGON_COLORS[0], 2.5, 1);
                if (ulMarkerSSS) {
            ...
            if (actualPointCenter_SSS) {
                // Solstice markers: 50% current size (radius 2.5), Fill Orange
                let centerMarkerSSS = drawIndividualPointMarker(actualPointCenter_SSS, "SSS Center", POLYGON_COLORS[0], POLYGON_COLORS[0], 2.5, 1);
                if (centerMarkerSSS) {
            ...
            if (actualPointLL_SSS) {
                // Solstice markers: 50% current size (radius 2.5), Fill Orange
                let llMarkerSSS = drawIndividualPointMarker(actualPointLL_SSS, "SSS Lower Limb", POLYGON_COLORS[0], POLYGON_COLORS[0], 2.5, 1);
                if (llMarkerSSS) {
            ...
            const wssIntersectionPoint = await processOrthodromeIntersection(wssZeroHorizonAzimuth, locationData, viewshedPolygon, "WSS", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_WSS) {
                // Solstice markers: 50% current size (radius 2.5), Fill Orange
                let ulMarkerWSS = drawIndividualPointMarker(actualPointUL_WSS, "WSS Upper Limb", POLYGON_COLORS[0], POLYGON_COLORS[0], 2.5, 1);
                if (ulMarkerWSS) {
            ...
            if (actualPointCenter_WSS) {
                // Solstice markers: 50% current size (radius 2.5), Fill Orange
                let centerMarkerWSS = drawIndividualPointMarker(actualPointCenter_WSS, "WSS Center", POLYGON_COLORS[0], POLYGON_COLORS[0], 2.5, 1);
                if (centerMarkerWSS) {
            ...
            if (actualPointLL_WSS) {
                // Solstice markers: 50% current size (radius 2.5), Fill Orange
                let llMarkerWSS = drawIndividualPointMarker(actualPointLL_WSS, "WSS Lower Limb", POLYGON_COLORS[0], POLYGON_COLORS[0], 2.5, 1);
                if (llMarkerWSS) {
            ...
            const erIntersectionPoint = await processOrthodromeIntersection(erZeroHorizonAzimuth, locationData, viewshedPolygon, "ER", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_ER) {
                ...
                let ulMarkerER = drawIndividualPointMarker(actualPointUL_ER, "ER Upper Limb", POLYGON_COLORS[1], POLYGON_COLORS[1], 2.5, 1);
            ...
            if (actualPointCenter_ER) {
                // Equinox markers: 50% current size (radius 2.5), Fill Yellow
                let centerMarkerER = drawIndividualPointMarker(actualPointCenter_ER, "ER Center", POLYGON_COLORS[1], POLYGON_COLORS[1], 2.5, 1);
            ...
            if (actualPointLL_ER) {
                // Equinox markers: 50% current size (radius 2.5), Fill Yellow
                let llMarkerER = drawIndividualPointMarker(actualPointLL_ER, "ER Lower Limb", POLYGON_COLORS[1], POLYGON_COLORS[1], 2.5, 1);
            ...
            const esIntersectionPoint = await processOrthodromeIntersection(esZeroHorizonAzimuth, locationData, viewshedPolygon, "ES", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_ES) {
                // Equinox markers: 50% current size (radius 2.5), Fill Yellow
                let ulMarkerES = drawIndividualPointMarker(actualPointUL_ES, "ES Upper Limb", POLYGON_COLORS[1], POLYGON_COLORS[1], 2.5, 1);
            ...
            if (actualPointCenter_ES) {
                // Equinox markers: 50% current size (radius 2.5), Fill Yellow
                let centerMarkerES = drawIndividualPointMarker(actualPointCenter_ES, "ES Center", POLYGON_COLORS[1], POLYGON_COLORS[1], 2.5, 1);
            ...
            if (actualPointLL_ES) {
                // Equinox markers: 50% current size (radius 2.5), Fill Yellow
                let llMarkerES = drawIndividualPointMarker(actualPointLL_ES, "ES Lower Limb", POLYGON_COLORS[1], POLYGON_COLORS[1], 2.5, 1);
            ...
            const ncqrIntersectionPoint = await processOrthodromeIntersection(ncqrZeroHorizonAzimuth, locationData, viewshedPolygon, "NCQR", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_NCQR) {
                ...
                let ulMarkerNCQR = drawIndividualPointMarker(actualPointUL_NCQR, "NCQR Upper Limb", POLYGON_COLORS[2], POLYGON_COLORS[2], 2.5, 1);
            ...
            if (actualPointCenter_NCQR) {
                // Cross Quarters markers: 50% current size (radius 2.5), Fill Green
                let centerMarkerNCQR = drawIndividualPointMarker(actualPointCenter_NCQR, "NCQR Center", POLYGON_COLORS[2], POLYGON_COLORS[2], 2.5, 1);
            ...
            if (actualPointLL_NCQR) {
                // Cross Quarters markers: 50% current size (radius 2.5), Fill Green
                let llMarkerNCQR = drawIndividualPointMarker(actualPointLL_NCQR, "NCQR Lower Limb", POLYGON_COLORS[2], POLYGON_COLORS[2], 2.5, 1);
            ...
            const scqrIntersectionPoint = await processOrthodromeIntersection(scqrZeroHorizonAzimuth, locationData, viewshedPolygon, "SCQR", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_SCQR) {
                // Cross Quarters markers: 50% current size (radius 2.5), Fill Green
                let ulMarkerSCQR = drawIndividualPointMarker(actualPointUL_SCQR, "SCQR Upper Limb", POLYGON_COLORS[2], POLYGON_COLORS[2], 2.5, 1);
            ...
            if (actualPointCenter_SCQR) {
                // Cross Quarters markers: 50% current size (radius 2.5), Fill Green
                let centerMarkerSCQR = drawIndividualPointMarker(actualPointCenter_SCQR, "SCQR Center", POLYGON_COLORS[2], POLYGON_COLORS[2], 2.5, 1);
            ...
            if (actualPointLL_SCQR) {
                // Cross Quarters markers: 50% current size (radius 2.5), Fill Green
                let llMarkerSCQR = drawIndividualPointMarker(actualPointLL_SCQR, "SCQR Lower Limb", POLYGON_COLORS[2], POLYGON_COLORS[2], 2.5, 1);
            ...
            const ncqsIntersectionPoint = await processOrthodromeIntersection(ncqsZeroHorizonAzimuth, locationData, viewshedPolygon, "NCQS", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_NCQS) {
                // Cross Quarters markers: 50% current size (radius 2.5), Fill Green
                let ulMarkerNCQS = drawIndividualPointMarker(actualPointUL_NCQS, "NCQS Upper Limb", POLYGON_COLORS[2], POLYGON_COLORS[2], 2.5, 1);
            ...
            if (actualPointCenter_NCQS) {
                // Cross Quarters markers: 50% current size (radius 2.5), Fill Green
                let centerMarkerNCQS = drawIndividualPointMarker(actualPointCenter_NCQS, "NCQS Center", POLYGON_COLORS[2], POLYGON_COLORS[2], 2.5, 1);
            ...
            if (actualPointLL_NCQS) {
                // Cross Quarters markers: 50% current size (radius 2.5), Fill Green
                let llMarkerNCQS = drawIndividualPointMarker(actualPointLL_NCQS, "NCQS Lower Limb", POLYGON_COLORS[2], POLYGON_COLORS[2], 2.5, 1);
            ...
            const scqsIntersectionPoint = await processOrthodromeIntersection(scqsZeroHorizonAzimuth, locationData, viewshedPolygon, "SCQS", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_SCQS) {
                // Cross Quarters markers: 50% current size (radius 2.5), Fill Green
                let ulMarkerSCQS = drawIndividualPointMarker(actualPointUL_SCQS, "SCQS Upper Limb", POLYGON_COLORS[2], POLYGON_COLORS[2], 2.5, 1);
            ...
            if (actualPointCenter_SCQS) {
                // Cross Quarters markers: 50% current size (radius 2.5), Fill Green
                let centerMarkerSCQS = drawIndividualPointMarker(actualPointCenter_SCQS, "SCQS Center", POLYGON_COLORS[2], POLYGON_COLORS[2], 2.5, 1);
            ...
            if (actualPointLL_SCQS) {
                // Cross Quarters markers: 50% current size (radius 2.5), Fill Green
                let llMarkerSCQS = drawIndividualPointMarker(actualPointLL_SCQS, "SCQS Lower Limb", POLYGON_COLORS[2], POLYGON_COLORS[2], 2.5, 1);
            ...
            const nmlrIntersectionPoint = await processOrthodromeIntersection(nmlrZeroHorizonAzimuth, locationData, viewshedPolygon, "NMLR", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_NMLR) {
                ...
                let ulMarkerNMLR = drawIndividualPointMarker(actualPointUL_NMLR, "NMLR Upper Limb", POLYGON_COLORS[3], POLYGON_COLORS[3], 2.5, 1);
            ...
            if (actualPointCenter_NMLR) {
                // Major Standstill markers: 50% current size (radius 2.5), Fill Dark Blue
                let centerMarkerNMLR = drawIndividualPointMarker(actualPointCenter_NMLR, "NMLR Center", POLYGON_COLORS[3], POLYGON_COLORS[3], 2.5, 1);
            ...
            if (actualPointLL_NMLR) {
                // Major Standstill markers: 50% current size (radius 2.5), Fill Dark Blue
                let llMarkerNMLR = drawIndividualPointMarker(actualPointLL_NMLR, "NMLR Lower Limb", POLYGON_COLORS[3], POLYGON_COLORS[3], 2.5, 1);
            ...
            const smlrIntersectionPoint = await processOrthodromeIntersection(smlrZeroHorizonAzimuth, locationData, viewshedPolygon, "SMLR", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_SMLR) {
                // Major Standstill markers: 50% current size (radius 2.5), Fill Dark Blue
                let ulMarkerSMLR = drawIndividualPointMarker(actualPointUL_SMLR, "SMLR Upper Limb", POLYGON_COLORS[3], POLYGON_COLORS[3], 2.5, 1);
            ...
            if (actualPointCenter_SMLR) {
                // Major Standstill markers: 50% current size (radius 2.5), Fill Dark Blue
                let centerMarkerSMLR = drawIndividualPointMarker(actualPointCenter_SMLR, "SMLR Center", POLYGON_COLORS[3], POLYGON_COLORS[3], 2.5, 1);
            ...
            if (actualPointLL_SMLR) {
                // Major Standstill markers: 50% current size (radius 2.5), Fill Dark Blue
                let llMarkerSMLR = drawIndividualPointMarker(actualPointLL_SMLR, "SMLR Lower Limb", POLYGON_COLORS[3], POLYGON_COLORS[3], 2.5, 1);
            ...
            const nmlsIntersectionPoint = await processOrthodromeIntersection(nmlsZeroHorizonAzimuth, locationData, viewshedPolygon, "NMLS", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_NMLS) {
                // Major Standstill markers: 50% current size (radius 2.5), Fill Dark Blue
                let ulMarkerNMLS = drawIndividualPointMarker(actualPointUL_NMLS, "NMLS Upper Limb", POLYGON_COLORS[3], POLYGON_COLORS[3], 2.5, 1);
            ...
            if (actualPointCenter_NMLS) {
                // Major Standstill markers: 50% current size (radius 2.5), Fill Dark Blue
                let centerMarkerNMLS = drawIndividualPointMarker(actualPointCenter_NMLS, "NMLS Center", POLYGON_COLORS[3], POLYGON_COLORS[3], 2.5, 1);
            ...
            if (actualPointLL_NMLS) {
                // Major Standstill markers: 50% current size (radius 2.5), Fill Dark Blue
                let llMarkerNMLS = drawIndividualPointMarker(actualPointLL_NMLS, "NMLS Lower Limb", POLYGON_COLORS[3], POLYGON_COLORS[3], 2.5, 1);
            ...
            const smnlsIntersectionPoint_major = await processOrthodromeIntersection(smnlsZeroHorizonAzimuth_major, locationData, viewshedPolygon, "SMNLS_Major", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_SMNLS) {
                // Major Standstill markers: 50% current size (radius 2.5), Fill Dark Blue
                let ulMarkerSMNLS = drawIndividualPointMarker(actualPointUL_SMNLS, "SMNLS Upper Limb", POLYGON_COLORS[3], POLYGON_COLORS[3], 2.5, 1);
            ...
            if (actualPointCenter_SMNLS) {
                // Major Standstill markers: 50% current size (radius 2.5), Fill Dark Blue
                let centerMarkerSMNLS = drawIndividualPointMarker(actualPointCenter_SMNLS, "SMNLS Center", POLYGON_COLORS[3], POLYGON_COLORS[3], 2.5, 1);
            ...
            if (actualPointLL_SMNLS) {
                // Major Standstill markers: 50% current size (radius 2.5), Fill Dark Blue
                let llMarkerSMNLS = drawIndividualPointMarker(actualPointLL_SMNLS, "SMNLS Lower Limb", POLYGON_COLORS[3], POLYGON_COLORS[3], 2.5, 1);
            ...
            const nmnlrIntersectionPoint = await processOrthodromeIntersection(nmnlrZeroHorizonAzimuth, locationData, viewshedPolygon, "NMNLR", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_NMNLR) {
                ...
                let ulMarkerNMNLR = drawIndividualPointMarker(actualPointUL_NMNLR, "NMNLR Upper Limb", POLYGON_COLORS[4], POLYGON_COLORS[4], 2.5, 1);
            ...
            if (actualPointCenter_NMNLR) {
                // Minor Standstill markers: 50% current size (radius 2.5), Fill Red
                let centerMarkerNMNLR = drawIndividualPointMarker(actualPointCenter_NMNLR, "NMNLR Center", POLYGON_COLORS[4], POLYGON_COLORS[4], 2.5, 1);
            ...
            if (actualPointLL_NMNLR) {
                // Minor Standstill markers: 50% current size (radius 2.5), Fill Red
                let llMarkerNMNLR = drawIndividualPointMarker(actualPointLL_NMNLR, "NMNLR Lower Limb", POLYGON_COLORS[4], POLYGON_COLORS[4], 2.5, 1);
            ...
            const smnlrIntersectionPoint = await processOrthodromeIntersection(smnlrZeroHorizonAzimuth, locationData, viewshedPolygon, "SMNLR", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_SMNLR) {
                // Minor Standstill markers: 50% current size (radius 2.5), Fill Red
                let ulMarkerSMNLR = drawIndividualPointMarker(actualPointUL_SMNLR, "SMNLR Upper Limb", POLYGON_COLORS[4], POLYGON_COLORS[4], 2.5, 1);
            ...
            if (actualPointCenter_SMNLR) {
                // Minor Standstill markers: 50% current size (radius 2.5), Fill Red
                let centerMarkerSMNLR = drawIndividualPointMarker(actualPointCenter_SMNLR, "SMNLR Center", POLYGON_COLORS[4], POLYGON_COLORS[4], 2.5, 1);
            ...
            if (actualPointLL_SMNLR) {
                // Minor Standstill markers: 50% current size (radius 2.5), Fill Red
                let llMarkerSMNLR = drawIndividualPointMarker(actualPointLL_SMNLR, "SMNLR Lower Limb", POLYGON_COLORS[4], POLYGON_COLORS[4], 2.5, 1);
            ...
            const nmnlsIntersectionPoint = await processOrthodromeIntersection(nmnlsZeroHorizonAzimuth, locationData, viewshedPolygon, "NMNLS", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_NMNLS) {
                // Minor Standstill markers: 50% current size (radius 2.5), Fill Red
                let ulMarkerNMNLS = drawIndividualPointMarker(actualPointUL_NMNLS, "NMNLS Upper Limb", POLYGON_COLORS[4], POLYGON_COLORS[4], 2.5, 1);
            ...
            if (actualPointCenter_NMNLS) {
                // Minor Standstill markers: 50% current size (radius 2.5), Fill Red
                let centerMarkerNMNLS = drawIndividualPointMarker(actualPointCenter_NMNLS, "NMNLS Center", POLYGON_COLORS[4], POLYGON_COLORS[4], 2.5, 1);
            ...
            if (actualPointLL_NMNLS) {
                // Minor Standstill markers: 50% current size (radius 2.5), Fill Red
                let llMarkerNMNLS = drawIndividualPointMarker(actualPointLL_NMNLS, "NMNLS Lower Limb", POLYGON_COLORS[4], POLYGON_COLORS[4], 2.5, 1);
            ...
            const smnlsIntersectionPoint_minor = await processOrthodromeIntersection(smnlsZeroHorizonAzimuth_minor, locationData, viewshedPolygon, "SMNLS_Minor", zeroHorizonIntersectionsLayerGroup);
            ...
            if (actualPointUL_SMNLS_minor) {
                // Minor Standstill markers: 50% current size (radius 2.5), Fill Red
                let ulMarkerSMNLS_minor = drawIndividualPointMarker(actualPointUL_SMNLS_minor, "SMNLS Upper Limb (Minor)", POLYGON_COLORS[4], POLYGON_COLORS[4], 2.5, 1);
            ...
            if (actualPointCenter_SMNLS_minor) {
                // Minor Standstill markers: 50% current size (radius 2.5), Fill Red
                let centerMarkerSMNLS_minor = drawIndividualPointMarker(actualPointCenter_SMNLS_minor, "SMNLS Center (Minor)", POLYGON_COLORS[4], POLYGON_COLORS[4], 2.5, 1);
            ...
            if (actualPointLL_SMNLS_minor) {
                // Minor Standstill markers: 50% current size (radius 2.5), Fill Red
                let llMarkerSMNLS_minor = drawIndividualPointMarker(actualPointLL_SMNLS_minor, "SMNLS Lower Limb (Minor)", POLYGON_COLORS[4], POLYGON_COLORS[4], 2.5, 1);
